<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!-- jstl 태그 라이브러리 사요을 위한 설정 : 아래 uri 는 톰캣10, 11버전 -->
...
<body>
	...
	<ul>
		...
		<li><a href="/jspProject/Search?category=B2">B2</a></li>
		<li><a href="/Search?category=B2">404 오류</a></li>
		<!-- / 를 붙이면 URL 이 / 부터 시작 -->
	</ul>
	<h3 style="margin-bottom: 0px;">키워드 검색</h3>
	<div>
	<!-- method="get" : 파라미터는 RUL 에 추가되어 전달된다. -->
	...
	<div>
		...
		<!--  empty 는 문자열 메소드 중에 null 과 빈문자열 검사합니다. -->
	...
	<table>
		...
		<!-- items 의 $ { } 기호는 list 이름 애트리뷰트값(request.getAttibute 실행 가져옵니다. -->
		<!-- dto 변수는 list 에서 하니씩 가져온 요소를 저장 -->
		<tr>
			<!-- status 변수는 forEach 에 실행정보 저장 -->
			<td>${status.count }</td>
			<td>${dto.pcode }</td>
			<td>${dto.category }</td>
			<td>${dto.pname }</td>
